...
addBtn.addEventListener('click', function (e) {
    ...
    let germe1_non_identifie = document.querySelector('#germe1_non_identifie').value;


    if (antibiotiqueValue === "") {
        alert('entrer la valeur de l\'antibiotique');
    }
    else if (germe1_identifie == "" && germe1_non_identifie == "") {
        alert('germe manquant!');
    } else {
        if (r1.checked == true) {
            if( germe1_identifie == ""){
                antiB_result.push({ "germe": germe1_non_identifie, "antibiotique": antibiotiqueValue, "resultat": true });
                dataSet.push([germe1_non_identifie, antibiotiqueValue, "sensible"]);
            }
            else {
        ...
        document.querySelector('#germe1_identifie').value = "";
        document.querySelector('#germe1_non_identifie').value = "";
        r1.checked = false;
        r2.checked = false;
        r3.checked = false;
